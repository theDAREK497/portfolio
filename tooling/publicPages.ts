...
export function renderPublicPage(
  ...
  const tools = `<nav class="static-tools" aria-label="${lang === 'ru' ? 'Версии страницы' : 'Page versions'}">${link(`./${homeFile(lang)}`, lang === 'ru' ? 'Портфолио' : 'Portfolio')} ${link(`./${sibling}`, lang === 'ru' ? 'English' : 'Русский')} ${resume ? `<button type="button" onclick="window.print()">${lang === 'ru' ? 'Печать / Сохранить PDF' : 'Print / Save PDF'}</button>` : link(`./resume-${lang}.html`, translations[lang].hero.resume)}</nav>`;
  html = html.replace(
